...
PROGMEM static const uint16_t const dqt_scales[8] = {
...
static uint32_t encode_callsign(char *callsign)
{
	...
	for(x = 0, c = callsign; x < SSDV_MAX_CALLSIGN && pgm_read_byte(c); x++, c++);
  ...
  uint8_t cval;
	...
	{
		x *= 40;
    cval = pgm_read_byte(c);
		if(cval >= 'A' && cval <= 'Z') x += cval - 'A' + 14;
		else if(cval >= 'a' && cval <= 'z') x += cval - 'a' + 14;
		else if(cval >= '0' && cval <= '9') x += cval - '0' + 1;
	}
